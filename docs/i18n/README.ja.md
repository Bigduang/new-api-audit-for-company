...
`token-audit` は [Bigduang/new-api-audit](https://github.com/Bigduang/new-api-audit) から署名付き監査イベントを受け取り、`request_id` で request event と usage event を結合し、ローカル SQLite に保存します。その後、リクエストが業務目的だったかを分類し、日次レポートを生成します。
